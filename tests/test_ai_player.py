...
class TestFirstMove:
    def test_empty_board_is_first_move(self):
        ai = AIPlayer()
        assert ai._is_first_move(_empty_board()) is True
...
class TestAnchorCells:
    def test_first_move_anchor_is_centre(self):
        ai = AIPlayer()
        anchors = ai._get_anchor_cells(_empty_board())
...
class TestScoring:
    ...
    def test_score_candidates_sorted_descending(self):
        ...
        low_scoring = [(0, 0, "A")]   # 1 point
        high_scoring = [(0, 0, "Z")]  # 10 points
        scored = ai._score_candidates(board, [low_scoring, high_scoring])
